#!/usr/bin/env python3
"""OLPR-kjernen — webserver (dashboard, API, ruter).

Importerbar: `from olpr_core import web; web.run()`. Homeserver utvider via
web.register_routes(...) før web.run() i steg 4 av felles-kjerne-planen.
"""
import hashlib
import html
import json
...
from olpr_core.config import (
    log, BASE_DIR, STATIC_DIR, TEMPLATE, LOGIN_TEMPLATE, SETTINGS_FILE,
    UKJENTE_DB, UKJENTE_SNAPSHOTS, LOGG_DB, SETTINGS_DEFAULTS,
    load_settings, save_settings_file, load_skilt, save_skilt, load_template,
)
...
def run(host="0.0.0.0", port=8080, auth=True):
    """Start dashboard-serveren. auth=False hopper over innlogging (homeserver bak Tailscale)."""
    global AUTH_ENABLED
    AUTH_ENABLED = auth
    if auth:
        migrate_auth()
